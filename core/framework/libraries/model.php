	...
	public function tableInfo($table){
		...
		if (file_exists(BASE_DATA_PATH.'/cache/fields/_pk.php')){
			...
			foreach ($full_table as $v_table) {
				if (substr($v_table[0],0,$count) != C('tablepre')) continue;
...
class ModelDb{
    ...
    // 查询表达式
    protected $selectSql  =     'SELECT%DISTINCT% %FIELD% FROM %TABLE%%INDEX%%JOIN%%WHERE% %LOCKROW% %GROUP%%HAVING%%ORDER%%LIMIT% %UNION%';
    ...
    public function parseSql($sql,$options=array()){
        $sql   = str_replace(
            array('%TABLE%','%DISTINCT%','%FIELD%','%JOIN%','%WHERE%','%GROUP%','%HAVING%','%ORDER%','%LIMIT%','%UNION%','%INDEX%','%LOCKROW%'),
            array(
    ...
    public function parseSql2($sql,$options=array()){
        $sql   = str_replace(
            array('%TABLE%','%DISTINCT%','%FIELD%','%JOIN%','%WHERE%','%GROUP%','%HAVING%','%ORDER%','%LIMIT%','%UNION%','%INDEX%','%LOCKROW%'),
            array(
                $this->parseTable($options),
...
//                    $whereStr .= ' ('.$key.' '.$val[1].') ';
	...
	public function parseAttr($options){
		if (isset($options['attr'])){
			if (in_array(isset($options['attr']),array('LOW_PRIORITY','QUICK','IGNORE','DELAYED','HIGH_PRIORITY','SQL_CACHE','SQL_NO_CACHE'))){
				return $options['attr'].' ';
			}
		}else{
			return '';
