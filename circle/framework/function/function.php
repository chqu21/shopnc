 ...
 * @return array
 */
function tidyThemeGoods($array, $key, $deep=1, $type='tiny'){
	if (is_array($array)){
		$tmp = array();
		foreach ($array as 	$v) {
			if($v['thg_type'] == 0){
				$v['image']		= thumb($v, $type);
				$v['thg_url']	= SHOP_SITE_URL.'/'.ncUrl(array('act'=>'goods','goods_id'=>$v['goods_id']), 'goods');
			}else{
...
function showMiniEditor($cname, $content = '', $type = 'all', $affix = array(), $gname = '', $goods = array(), $readperm = array(), $rpvalue = 0){
	switch ($type){
		...
		case 'hQuickReply':
			...
			break;
		default:
			$items = array('font', 'size', 'line', 'bold', 'italic', 'underline', 'color', 'line', 'affix', 'line', 'url', 'flash', 'goods', 'line', 'smilier');
	...
	$_goods	= C('taobao_api_isuse') ? "<a href=\"javascript:void(0);\" nctype=\"chooseGoods\" title=\"".L('nc_insert_relevance_goods')."\"><i class=\"url-goods\"></i>".L('nc_goods')."</a>" : "";
