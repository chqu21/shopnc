...
class themeControl extends BaseCircleThemeControl{
	...
	public function ajax_quickreplyOp(){
		...
		$data['member_avatar'] = getMemberAvatarForCircle($_SESSION['avatar']); // 头像
		// 回复
		...
		if(!empty($reply_list)){
			foreach($reply_list as $key=>$val){
				$reply_list[$key]['member_avatar'] = getMemberAvatarForCircle($val['member_avatar']);
	...
	public function save_themeOp(){
		if(chksubmit()){
			...
			$insert['member_avatar']= md5($_SESSION['member_id']).'.jpg';
	...
	public function check_linkOp(){
		...
		if(!$domain) {echo 'false'; exit;}
		switch ($domain){
			case 'taobao.com':
			case 'tmall.com':
				if(C('taobao_api_isuse')){
					$goods_info = $this->getTaobaoGoodsInfo($link);
				}else{
					echo 'false';exit;
				}
				break;
			default:
				$goods_info = $this->getSelfGoodsInfo($link);
				break;
		}
		if($goods_info){
	...
	private function getSelfGoodsInfo($link){
		...
		if(isset($array['query'])){
			...
			$id = preg_replace(array('/\/goods-(\d+)-(\d+)\.html/i','/\/goods-(\d+)\.html/i'), '$1', $array['path']);
		}
		...
		$return['img']		= thumb($goods_info, 'tiny');
