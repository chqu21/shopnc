...
session_save_path(BASE_DATA_PATH.DS.'cache'.DS.'session');
require_once(BASE_DATA_PATH.DS.'config'.DS.'config.ini.php');
if(!empty($config) && is_array($config)){

	$sitepath = strtolower(substr($_SERVER['PHP_SELF'], 0, strrpos($_SERVER['PHP_SELF'], '/')));
	$tmp_array = explode('/',$sitepath);
	$site_url = strtolower('http://'.$_SERVER['HTTP_HOST'].implode('/',$tmp_array));
